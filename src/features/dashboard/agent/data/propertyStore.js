...
export const getAllProperties = () => readStore();

export const getAgentProperties = (agentId) => {
  const allProperties = readStore();
  return allProperties.filter((property) => property.agentId === agentId);
};
...
export const addProperty = (propertyData) => {
  ...
  const newProperty = {
    ...propertyData,
    id: Math.max(...allProperties.map((property) => property.id), 0) + 1,
    views: 0,
    inquiries: 0,
...
export const deleteProperty = (propertyId) => {
  const allProperties = readStore();
  const updated = allProperties.filter((property) => property.id !== propertyId);
  writeStore(updated);
  return updated;
};
...
export const getAgentStats = (agentId) => {
  ...
  return {
    ...
    active: properties.filter((property) => property.status === 'verified' || property.status === 'active').length,
    ...
    totalViews: properties.reduce((sum, property) => sum + (property.views || 0), 0),
    totalInquiries: properties.reduce((sum, property) => sum + (property.inquiries || 0), 0),
  };
};
